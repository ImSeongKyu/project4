...
window.onload = function() {
  ...
  function checkID() {
    ...
    if (id.value.trim().length == 0) {
      ...
      $.ajax({
        url: './the_checkID.do', // spring execute, http://localhost:9091/member/the_checkID.do?id=user1@gmail.com
        type: 'get',  // post
  ...
  function send() { // 회원 수정 처리
      ...
      if (id.value.trim().length == 0) {
        id_msg.innerHTML= 'ID가 누락됬습니다. ID(이메일)는 3자이상 권장합니다.';
        id_msg.classList.add('span_warning');    // class 적용
        id.focus();
...
<c:import url="/menu/top.do" />
  <DIV class='title_line'>회원 정보 수정(*: 필수)</DIV>
  ...
  <FORM name='frm' id='frm' method='POST' action='./the_update.do' class="">
    ...
    <div class="form-floating mb-1 mt-3" style="width: 60%; margin:0px auto;">
      <input type="text" class="form-control" name='id' id='id' value='${thememberVO.id }' required="required" placeholder="아이디*" autofocus="autofocus">
    ...
    <div class="form-floating mb-1 mt-3" style="width: 60%; margin:0px auto;">
      <input type="text" class="form-control" name='mname' id='mname' value='${thememberVO.mname }' required="required" placeholder="성명*" autofocus="autofocus">
      <label for="mname">성명*</label>
      
    </div>
    <div class="form-floating mb-1 mt-3" style="width: 60%; margin:0px auto;">
      <input type="text" class="form-control" name='tel' id='tel' value='${thememberVO.tel }' required="required" placeholder="전화 번호" autofocus="autofocus">
      <label for="tel">전화 번호</label>
      
    </div>
    <div class="form-floating mb-1 mt-3" style="width: 60%; margin:0px auto;">
      <input type="text" class="form-control" name='zipcode' id='zipcode' value='${thememberVO.zipcode }' required="required" placeholder="우편번호" autofocus="autofocus">
...
<%--    <div class="form-group"> 
      <label>아이디*:
        <input type='text' class="form-control form-control-sm" name='id' id='id' value='${thememberVO.id }' required="required" placeholder="아이디*" autofocus="autofocus">
      </label>
      <button type='button' id="btn_checkID" onclick="checkID()" class="btn btn-primary btn-sm">중복확인</button>
      <span id='id_msg'></span>
    </div> 
    ...
    <div class="form-group"> 
      <label>성명*:
        <input type='text' class="form-control form-control-sm" name='mname' id='mname' value='${thememberVO.mname }' required="required" placeholder="성명">
      </label>
    ...
    <div class="form-group">  label의 크기에따라 input 태그의 크기가 지정되는 형태, 줄이 변경되지 않는 패턴 
      <label>전화 번호:
        <input type='text' class="form-control form-control-sm" name='tel' id='tel' value='${thememberVO.tel }' required="required" placeholder="전화번호">
      </label>
      예) 010-0000-0000
    </div>   
    ...
    <div class="form-group"> 
      <label>우편 번호:
        <input type='text' class="form-control form-control-sm" name='zipcode' id='zipcode' value='${thememberVO.zipcode }' placeholder="우편번호">
      </label>
      <button type="button" id="btn_DaumPostcode" onclick="DaumPostcode()" class="btn btn-primary btn-sm">우편번호 찾기</button>
    </div>  
    ...
    <div class="form-group">
      ...
      <input type='text' class="form-control form-control-sm" name='address1' id='address1' value='${thememberVO.address1 }' placeholder="주소">
    </div>   

    <div class="form-group">
      <label style="width: 100%;">상세 주소:</label>
      <input type='text' class="form-control form-control-sm" name='address2' id='address2' value='${thememberVO.address2 }' placeholder="상세 주소">
    </div>   --%>
