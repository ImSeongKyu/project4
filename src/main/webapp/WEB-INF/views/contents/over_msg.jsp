...
<c:set var="catenum" value="${param.catenum }" /> <%-- mav.addObject("cateno", contentsVO.getCateno()); // redirect parameter 적용 --%>
  ...
  <fieldset class='fieldset_basic'>
    <UL>
      <c:choose>
        ...
        <c:when test="${code == 'check_upload_file_fail'}"> <%-- Java if --%>
          <li class='li_none'>
            ...
            <span>가능한 파일 형식[jpeg, jpg, jpef, png, gif, txt, hwp, doc, ppt, pptx, xls, xlsx, zip, tar, gz, ipynb]</span>
          </li> 
        </c:when>
      ...
      <LI class='li_none'>
        ...
        <button type='button' onclick="location.href='./over_create.do?catenum=${catenum}'" class="btn btn-info btn-sm">새로운 컨텐츠 등록</button>
        <button type='button' onclick="location.href='./over_list_by_cateno.do?catenum=${catenum}'" class="btn btn-info btn-sm">목록</button>
        <button type='button' onclick="location.href='./over_list_by_cateno_grid.do?catenum=${catenum}'" class="btn btn-info btn-sm">갤러리 목록</button>
      </LI>
    </UL>
  </fieldset>
