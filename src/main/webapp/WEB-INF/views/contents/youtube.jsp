...
<c:import url="/menu/top.do" />
  <DIV class='title_line'><A href="/contents/over_list_by_cateno.do?catenum=${procateVO.catenum }" class="title_link">${cateVO.name }</A> > ${contentsVO.title } > Youtube 등록/수정/삭제</DIV>

  <ASIDE class="aside_right">
    <A href="javascript:location.reload();">새로고침</A>
    <span class='menu_divide' >│</span>    
    <A href="./over_list_by_cateno.do?catenum=${param.catenum }&now_page=${param.now_page == null ? 1 : param.now_page}&word=${param.word }">기본 목록형</A>    
    <span class='menu_divide' >│</span>
    ...
    <form name='frm' id='frm' method='get' action='./over_list_by_cateno.do'>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' class='btn btn-secondary btn-sm' 
                    onclick="location.href='./over_list_by_cateno.do?catenum=${procateVO.catenum}&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>   
  ...
  <FORM name='frm_youtube' method='POST' action='./youtube.do'>
    <input type="hidden" name="overcontentsno" value="${param.overcontentsno }">
    
    <div>
       <label>Youtube 스크립트</label>
       <textarea name='youtube' class="form-control" rows="12" style='width: 100%;'>${overcontentsVO.youtube }</textarea>
    </div>
